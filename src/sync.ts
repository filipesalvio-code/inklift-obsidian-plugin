/** InkLift vault sync engine */
import type { App, TFile } from "obsidian";
import type { InkLiftAPI } from "./api";
import type { SyncedNote } from "./types";
const STORAGE_KEY_LAST_SYNC = "inklift-last-sync";
export class SyncEngine {
  constructor(private app: App, private api: InkLiftAPI, private syncFolder: string, private includeSourceImages: boolean) {}
  getLastSync(): string | null { return this.app.loadLocalStorage(STORAGE_KEY_LAST_SYNC); }
  setLastSync(iso: string): void { this.app.saveLocalStorage(STORAGE_KEY_LAST_SYNC, iso); }
  async run(): Promise<{ synced: number; conflicts: number; errors: string[] }> {
    ...
    for (const note of response.notes) {
      try {
        ...
        else if (result !== "skipped") synced++;
      } catch (e) { errors.push(note.notebook_name + " p" + note.page_number + ": " + String(e)); }
    }
    this.setLastSync(response.server_time);
    return { synced, conflicts, errors };
  }
  private async writeNoteToVault(note: SyncedNote): Promise<"created" | "updated" | "conflict" | "skipped"> {
    ...
    if (this.includeSourceImages && note.source_image_path) await this.downloadSourceImage(note, folder);
    const content = note.markdown;
    const existing = this.app.vault.getAbstractFileByPath(path);
    if (existing instanceof TFile) {
      const existingContent = await this.app.vault.read(existing);
      const lastServerUpdate = this.extractFrontmatterField(existingContent, "inklift_server_updated_at");
      if (lastServerUpdate && note.server_updated_at && lastServerUpdate === note.server_updated_at) return "skipped";
      ...
      if (existingBody !== newBody && existingBody.trim() !== "") {
        ...
        const conflictPath = path.replace(".md", " (InkLift conflict " + dateStr + ").md");
        if (!this.app.vault.getAbstractFileByPath(conflictPath)) {
  ...
  private sanitizeFilename(name: string): string { return name.replace(/[<>:"\/\\|?*]/g, "_").trim() || "Untitled"; }
  private async downloadSourceImage(note: SyncedNote, folder: string): Promise<void> {
