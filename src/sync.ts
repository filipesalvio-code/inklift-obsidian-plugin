...
export class SyncEngine {
  ...
  private async writeNoteToVault(
    ...
    // note.markdown already contains YAML frontmatter from the backend
  ...
  private async downloadSourceImage(
    ...
    try {
      ...
      new Notice(`InkLift: Could not download source image for ${note.notebook_name} p${note.page_number + 1}`);
    }
  }
}
